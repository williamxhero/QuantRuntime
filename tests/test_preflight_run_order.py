...
def test_confirmed_conformance_preflight_identity_skips_conformance_relaunch(
    monkeypatch, tmp_path: Path
) -> None:
    events: list[str] = []
    conformance_ref = {"conformance_id": "sha256:" + "c" * 64}
    ...
    class Client:
        ...
        def submit_run(self, request):
            events.append("submit_run")
            assert request["behavioral_conformance"] == conformance_ref
            return {"run_id": "reused-conformance"}

    class Worker:
        def __init__(self, workspace):
            del workspace

    class ForbiddenConformance:
        def __init__(self, *args, **kwargs):
            del args, kwargs
            raise AssertionError("confirmed conformance must not relaunch")
    ...
    class Preflight:
        ...
        def preflight(self, draft):
            events.append("preflight")
            assert draft["schema"] == "quant-research.runtime-preflight-request.v3"
            assert draft["behavioral_conformance"] == conformance_ref
            return {"status": "accepted", "frozen_snapshot": {"snapshot_id": "new-data"}}
    ...
    monkeypatch.setattr(cli, "WorkspaceClient", Client)
    monkeypatch.setattr(cli, "WorkspaceWorker", Worker)
    monkeypatch.setattr(cli, "RuntimeConformance", ForbiddenConformance)
    monkeypatch.setattr(cli, "RuntimePreflight", Preflight)
    monkeypatch.setattr(cli, "RuntimeExecutor", Executor)
    request_path = tmp_path / "request.json"
    request_path.write_text(
        json.dumps(
            {
                ...
                "strategy_package": {},
                "snapshot_request": {"snapshot_id": "new-data"},
                "parameters": {},
                "sandbox_profile": {},
                "behavioral_conformance": conformance_ref,
                "execution": {},
            }
        ),
        encoding="utf-8",
    ...
    assert result["status"] == "completed"
    assert events == ["preflight", "submit_run", "attempt"]
